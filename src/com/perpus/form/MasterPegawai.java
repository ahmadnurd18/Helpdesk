...
public class MasterPegawai extends javax.swing.JPanel {
    ...
    private void setLayoutForm() {
        ...
        txtTanggal.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "yyyy-MM-dd");
    }
...
int row = tblData.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih pegawai yang akan dicetak");
            return;
        }
 ...
 private void paginationAnggota() {
    ...
    public void getData(int startIndex, int entriesPage, DefaultTableModel model) {
        model.setRowCount(0);
        try {
            String sql = "SELECT * FROM pegawai ORDER BY ID_Pegawai LIMIT ?, ?";
            try (PreparedStatement st = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    java.sql.Date tgl = rs.getDate("Tanggal_Bergabung");
                    String tglStr = tgl != null ? sdf.format(tgl) : "";
                    model.addRow(new Object[]{ " " + no++, id, nama, email, telepon, jk, tglStr });
                }
            }
        } catch (SQLException e) {
            Logger.getLogger(MasterPegawai.class.getName()).log(Level.SEVERE, null, e);
    ...
    private String setIDPegawai() {
        String urutan = "PGW2511001";
        String sql = "SELECT RIGHT(ID_Pegawai, 3) AS Nomor FROM pegawai WHERE ID_Pegawai LIKE 'PGW2511%' ORDER BY ID_Pegawai DESC LIMIT 1";
        try (PreparedStatement st = conn.prepareStatement(sql);
             ResultSet rs = st.executeQuery()) {
            if (rs.next()) {
                int nomor = Integer.parseInt(rs.getString("Nomor")) + 1;
                urutan = "PGW2511" + String.format("%03d", nomor);
            }
    ...
    private void insertData() {
        ...
        if (id.isEmpty() || nama.isEmpty() || email.isEmpty() || tgl.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Lengkapi semua field!", "Validasi", JOptionPane.WARNING_MESSAGE);
            return;
        }

        try {
            String sql = "INSERT INTO pegawai (ID_Pegawai, Nama_Pegawai, Email, Telepon, Jenis_Kelamin, Tanggal_Bergabung) VALUES (?,?,?,?,?,?)";
            try (PreparedStatement st = conn.prepareStatement(sql)) {
                st.setString(1, id);
                st.setString(2, nama);
                ...
                st.setString(5, jk);
                st.setDate(6, java.sql.Date.valueOf(tgl));
            ...
            JOptionPane.showMessageDialog(this, "Gagal simpan: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void updateData() {
        ...
        try {
            String sql = "UPDATE pegawai SET Nama_Pegawai=?, Email=?, Telepon=?, Jenis_Kelamin=?, Tanggal_Bergabung=? WHERE ID_Pegawai=?";
            try (PreparedStatement st = conn.prepareStatement(sql)) {
                st.setString(1, nama);
                st.setString(2, email);
                st.setString(3, telepon);
                st.setString(4, jk);
                st.setDate(5, java.sql.Date.valueOf(tgl));
                st.setString(6, id);
                if (st.executeUpdate() > 0) {
    ...
    private void searchData() {
        ...
        model.setRowCount(0);
        try {
            String sql = "SELECT * FROM pegawai WHERE Nama_Pegawai LIKE ? OR Email LIKE ? ORDER BY ID_Pegawai";
            try (PreparedStatement st = conn.prepareStatement(sql)) {
                st.setString(1, "%" + key + "%");
                st.setString(2, "%" + key + "%");
                ResultSet rs = st.executeQuery();
